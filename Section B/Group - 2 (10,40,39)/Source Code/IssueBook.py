...
def issue():
    ...
    extractbook_no = "select book_no from "+bookTable
    try:
        ...
        if book_no in allbook_no:
            ...
            messagebox.showinfo("Error", "Book no. not present")
    except:
        messagebox.showinfo("Error", "Can't fetch Book no.")

    issueSql = "insert into "+issueTable + \
        " values ('"+book_no+"','"+issueto+"')"
    show = "select * from "+issueTable

    updateStatus = "update "+bookTable + \
        " set status = 'issued' where book_no = '"+book_no+"'"
    try:
